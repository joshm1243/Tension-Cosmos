def PLM_SA_GENERATE(prompt):
    ...
    ## Also removing any generated outputs as not to confuse the future model
    if os.path.exists("./services/samples/outputs"):
        shutil.rmtree("./services/samples/outputs")
    os.mkdir("./services/samples/outputs")
    ...
    dalle_path = Path("./services/samples/model/dalle.pt")
    ...
    for j, text in tqdm(enumerate(texts)):
        ...
        outputs_dir = Path("./services/samples/outputs") / file_name.replace(' ', '_')[:(100)]
