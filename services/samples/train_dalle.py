...
def PLM_SA_TRAIN_DALLE():
    ...
    ## Removing any pre-trained DALLE models
    if os.path.exists("./services/samples/model/dalle.pt"):
        os.remove("./services/samples/model/dalle.pt")
    ...
    DALLE_OUTPUT_FILE_NAME = "./services/samples/model/dalle.pt"
    IMAGE_TEXT_FOLDER = "./services/samples/renders"
    VAE_PATH = "./services/samples/model/vae-final.pt"
    ...
    if not ENABLE_WEBDATASET:
        ...
        if Path(IMAGE_TEXT_FOLDER).is_dir():
            DATASET = [str(p) for p in Path(IMAGE_TEXT_FOLDER).glob("**/*") if ".tar" in str(p).lower()] # .name
            assert len(DATASET) > 0, 'The directory ({}) does not contain any WebDataset/.tar files.'.format(IMAGE_TEXT_FOLDER)
            print('Found {} WebDataset .tar(.gz) file(s) under given path {}!'.format(len(DATASET), IMAGE_TEXT_FOLDER))
        elif ('http://' in IMAGE_TEXT_FOLDER.lower()) | ('https://' in IMAGE_TEXT_FOLDER.lower()):
            DATASET = f"pipe:curl -L -s {IMAGE_TEXT_FOLDER} || true"
            print('Found {} http(s) link under given path!'.format(len(DATASET), IMAGE_TEXT_FOLDER))
        elif 'gs://' in IMAGE_TEXT_FOLDER.lower():
            DATASET = f"pipe:gsutil cat {IMAGE_TEXT_FOLDER} || true"
            print('Found {} GCS link under given path!'.format(len(DATASET), IMAGE_TEXT_FOLDER))
        elif '.tar' in IMAGE_TEXT_FOLDER:
            DATASET = IMAGE_TEXT_FOLDER
            print('Found WebDataset .tar(.gz) file under given path {}!'.format(IMAGE_TEXT_FOLDER))
        else:
            raise Exception('No folder, no .tar(.gz) and no url pointing to tar files provided under {}.'.format(IMAGE_TEXT_FOLDER))
